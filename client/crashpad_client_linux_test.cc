...
void ValidateAttachment(const CrashReportDatabase::UploadReport* report) {
  ...
  ASSERT_EQ(memcmp(kTestAttachmentContent, buf, sizeof(kTestAttachmentContent)), 0);
}

void ValidateDump(const CrashReportDatabase::UploadReport* report, bool start_at_crash) {
